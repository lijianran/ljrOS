...
extern char trampoline[];  // trampoline.S 内核中断 trap

/*
 * create a direct-map page table for the kernel. 一对一直接映射到内核页表中
 */
void kvminit() {
 ...
 * @param level
 */
void _helper_vmprint(pagetable_t pagetable, int level) {
    for (int i = 0; i < 512; ++i) {
        // 取 pte
        pte_t pte = pagetable[i];
...
/**
 * @brief 为每个进程初始化 kernel page table
 *
 * @return pagetable_t
 */
pagetable_t proc_kernelpt_init() {
    pagetable_t kpt;
    kpt = uvmcreate();
    if (kpt == 0)
        return 0;
    ...
    uvmmap(kpt, (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext,
           PTE_R | PTE_W);
 ...
 * @param newsz
 */
void u2kvmcopy(pagetable_t pagetable, pagetable_t kpagetable, uint64 oldsz,
               uint64 newsz) {
